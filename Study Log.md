...
## 1-26
 - 期权通过提前购买标的股票的权力 到期日再买卖赚差价 
 - 整体盈利为减权金-行权价x+到期日股价
 - 通过提交保证金 提前售卖期权

## 1-28
 - Transformer 1.词嵌入 2.位置嵌入 3.自注意力 4.全连接网络 5.输出层
 - 将输入的句子拆分为词即token 不同token之间通过维度划分 在维度上会给一组向量 可以运用计算得出token之间的关系 向量还代表着位置信息
 - 对于每个词key都包含背后的多词义库value 计算机通过用户输入的查询query 计算key并获得value
 - 刚开始先让token赋值的向量与设置好的权重矩阵相乘 再结果矩阵拆分三个向量 分别对应QKV
 - 通过每个Q都与每个K作点积 判断两个向量方向的关联性 接着为了方便计算与关注差异 在此基础上在除以向量维度的开方 最后会将结果矩阵的行做一次softmax即权重分配
 - 将运算好的比例矩阵与v向量加权 相当于对一个词在不同的数据库中应该拿多少比例的数据信息
 - 但是这样只做一次会导致单一性 因此计算机会多次重复计算并叠加 即多头注意力 最终的结果通过一个向量表达出来
 - 计算机会将初始输入与上面计算结果相加 以确保原token的信息不会被覆盖即残差网络add
 - 接着为了使输出为0/1 会进行统一调整即归一化norm
 - 为了捕捉复杂的语义关联，拟合自然语言排布里单一方向的语义关系 通过前馈网络FFN的EeLU激活函数 将输入向量与输出向量的映射向量掰弯 即非线性曲线接着重复一次add&norm
 - 以上操作称为编码层 可以让模型上下文理解 多个编码层堆叠即为编码器 输出为包含整体语义理解的向量
 - 最后通过翻译<Bos>翻译成英文 <Bos>后面接翻译的第一个单词 由上面输出的向量预测最合理的单词 
 - 1.为确保翻译的上下词的连通性 再次进行注意力运算 Q是下一个单词N K是上一个的单词N-1 即带掩码的多头自注意力
 - 2.为确保翻译的词应该参考于输出部分的意义 也是注意力 Q是未知词 KV是输出中的KV 即多头交叉注意力 
 - 1 2 都进行add&norm 将输出再一次FFN 最后再add&norm
 - 以上操作为单个解码层 多个堆叠即解码器
 - 整体流程为输入 编码 解码 输出 其中的编码和解码一个负责理解 一个负责生成 两个构成经典的架构
 - 对于量化策略的改进 分为原基础上改进 与 重新改造

## 1-29
 - 通过前k个为参考样本 如果k+n个时 比参考样本好 则选择k+n